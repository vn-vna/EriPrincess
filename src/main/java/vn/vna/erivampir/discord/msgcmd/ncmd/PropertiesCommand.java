...
public class PropertiesCommand extends CommandTemplate {
    ...
    public void invoke(String[] commands, MessageReceivedEvent event) {
        ...
        Optional<DiscordGuildConfig> guildConfigSR = dscGuildCfgRepositoryH2.findOne(Example.of(example));
        ...
        if (guildConfigSR.isPresent()) {
            embedBuilder
                ...
                .addField("Registered on", guildConfigSR.get().getRegisteredDate().toString(), false)
                .addField("Server timezone", guildConfigSR.get().getGmt().toString(), false);
        } else {
            embedBuilder
